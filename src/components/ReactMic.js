...
export default class ReactMic extends Component {
  ...
  componentDidMount() {
    const { onStop, onStart,onProgress, audioElem, audioBitsPerSecond, mimeType } = this.props;
    ...
    if(audioElem) {
      ...
      this.setState({
        analyser            : analyser,
        microphoneRecorder  : new MicrophoneRecorder(onStart, onStop,onProgress, options),
        canvas              : canvas,
        canvasCtx           : canvasCtx
      }, () => {
        this.visualize();
